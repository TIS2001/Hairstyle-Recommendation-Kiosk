...
## UI 화면 구성
main 내에서 MainUI 클래스를 호출하여 UI 인스턴스를 생성한다. 특정 페이지는 초기화 작업 중에 프레임을 미리 생성하고, 프레임을 불러올 때 tkinter 모듈에서 제공하는 tkraise()를 호출하여 가장 앞 화면으로 올라오게 된다. 사용자가 입력, 선택, 받기를 수행하는 프레임은 미리 생성하지 않고, 순차적으로 진행함에 따라 실행하게 된다. 사용자 인터페이스를 구성하는 데 사용되는 메서드, 함수, 라이브러리의 API 호출, UI 위젯의 생성 및 설정, 이벤트 핸들러 함수 등은 아래와 같다. 


![Total_Pages](https://github.com/TIS2001/Hairstyle-Recommendation-Kiosk/assets/94544462/824cadf7-b5e0-4b57-8a41-e1ab313c246b)



### Frame_init 메서드: 프레임 초기 설정
* 카메라 초기화, 스타트 프레임, 로그인 프레임, 서비스 선택 프레임을 시작
* 스타트 프레임을 가장 위에 띄움

### open_win1 메서드: 로그인과 회원가입 선택 페이지
* 회원가입 버튼을 클릭하면 open_win2을, 로그인 버튼을 클릭하면 open_win3을 실행
